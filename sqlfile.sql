DROP DATABASE IF EXISTS HealthcareDatabase;
...
CREATE TABLE Healthcare (
  ...
  Date_of_Admission VARCHAR(20),   -- keep as text for now
  Doctor VARCHAR(150),
  ...
  Discharge_Date VARCHAR(20),      -- keep as text for now
  Medication VARCHAR(150),
  Test_Results VARCHAR(50)
);

#Loading the .csv file. 
#NOTE: 'C:/ProgramData/MySQL/MySQL Server 8.0/Uploads/' Move to this secure location then only transfer allowed. 
...
#Converting the dd-mm-yyyy to the date datatype (YYYY-MM-DD) 
#%d-%m-%Y -> since the data is in DD-MM-YYYY it is parsed into date datatype.  %d-%m-%Y -> this guides that parsing from the text 
UPDATE Healthcare
